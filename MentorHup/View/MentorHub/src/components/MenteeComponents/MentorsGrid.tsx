...
const MentorsGrid = () => {
  ...
  const [modalViewProfile, setViewProfile] = useState<{
    mentor: Mentor | null;
    show: boolean;
  }>({
    mentor: null,
  ...
  const onViewProfile = (mentor: Mentor) => {
    setViewProfile({ mentor, show: true });
  };
  ...
  const onCloseModalView = () => {
    setViewProfile((prev) => ({ ...prev, show: false }));
  };
  ...
  const getMentors = useCallback(
    async (
      pageNumber: number,
      pageSize: number,
      apiFilters: Partial<MentorFilters> = {},
      retryCount = 0
    ) => {
      const MAX_RETRIES = 3;
      ...
      if (apiFilters.experiences)
        filterParams.experiences = apiFilters.experiences;
      ...
      try {
        console.log(
          `🔧 [${requestId}] Using mentorService with backend pagination and filters...`
        );
        ...
        console.log(
          `✅ [${requestId}] Transformed ${transformedMentors.length} mentors`
        );
        ...
        setTotalPages(
          result.totalPages ||
            Math.ceil(
              (result.totalItems || transformedMentors.length) / pageSize
            )
        );
      } catch (error: any) {
  ...
  return (
    <>
      ...
      <ModalProfile
        ...
        mentor={modalViewProfile.mentor}
        isOpen={modalViewProfile.show}
      />
                      ...
                      <div className="flex space-x-2">
                        ...
                        >
                          <SiReaddotcv
                            ...
                            onClick={
                              mentor.cvLink
                                ? () => window.open(mentor.cvLink, "_blank")
                                : undefined
                            }
                          />
                        </button>
                          ...
                          <div className="space-y-1.5">
                            {mentor.availabilities
                              ...
                              .map((availability, index) => (
                                <div
                                  key={
                                    availability.mentorAvailabilityId || index
                                  }
                                ...
                                >
                                  ...
                                  <div className="text-xs text-gray-500 mt-1.5 flex items-center">
                                    ...
                                    {availability.durationInMinutes} min session
                                    • Available now
                                  </div>
                                </div>
                              ))}
                            {mentor.availabilities.filter((a) => !a.isBooked)
                              .length > 1 && (
                              <div
                      ...
                      >
                        {isDots ? (
                          ...
                          <motion.button
                            ...
                            disabled={isActive}
                            className={`px-3 py-1.5 mx-0.5 rounded-md text-sm font-medium transition-all duration-200 min-w-[36px] border ${
