...
import { Star, Users, Heart, Share2, ChevronLeft, ChevronRight } from 'lucide-react';
...
const mockMentors: Mentor[] = Array.from({ length: 25 }, (_, i) => ({
  id: `mentor-${i + 1}`,
  name: `Mentor ${i + 1}`,
  title: `Senior ${['Engineer', 'Designer', 'Manager', 'Analyst', 'Developer'][i % 5]}`,
  ...
  specialties: ['React', 'Node.js', 'Python', 'System Design', 'Leadership'].slice(0, Math.floor(Math.random() * 3) + 2),
  isOnline: Math.random() > 0.5
}));

const MentorsGrid = ({ mentors = mockMentors, onBookSession = (mentor) => console.log('Book session with:', mentor.name) }: MentorsGridProps) => {
              ...
              <div className="p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-semibold text-white ${
                      mentor.isOnline ? 'bg-gradient-to-r from-green-500 to-green-600' : 'bg-gradient-to-r from-gray-500 to-gray-600'
                    }`}>
                      {mentor.avatar}
                    </div>
                    <div>
                      <h3 className={`font-semibold text-lg ${textClass}`}>{mentor.name}</h3>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    <div className="flex items-center">
                      ...
                      <span className={`ml-1 text-sm font-medium ${textClass}`}>{mentor.rating.toFixed(1)}</span>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div className="text-center">
                    <div className={`text-lg font-semibold ${textClass}`}>{mentor.experience}</div>
                    <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Experience</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-lg font-semibold ${textClass}`}>{mentor.totalSessions}</div>
                    <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Sessions</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-lg font-semibold ${textClass}`}>${mentor.hourlyRate}</div>
                    <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Per Hour</div>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-semibold text-green-600">{mentor.successRate}%</span>
                  </div>
          ...
          <div className="flex items-center justify-center space-x-2">
            ...
            <div className="flex items-center space-x-1">
              {getPageNumbers().map((page, index) => (
                <div key={index}>
                  {page === '...' ? (
                    <span className={`px-3 py-2 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                      ...
                    </span>
                  ) : (
                    <motion.button
